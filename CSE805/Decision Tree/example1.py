...
# ROOT NODE: represents the entire population or sample that gets divided into two or more homogeneous sets
...
# Entropy: measures the randomness of a class = p(yes)log2(p(yes)) - p(no)log2(p(no))

# BRANCH/SUBTREE: represents the outcome of a decision or test, leading to another node
...
class Question:
    ...
    is used to compare the feature value in an example to the feature value stored in the question.
    """
...
def find_best_split(rows):
    ...
    n_features = len(rows[0]) - 1  # Number of columns excluding the label column
